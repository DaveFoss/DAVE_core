...
def create_pp_ehvhv_lines(
    ...
    if isinstance(lines.iloc[0].from_node, str):
        lines["from_bus"] = lines.from_node.apply(
            lambda x: net.bus[net.bus["name"] == x].index[0]
        )
        lines["to_bus"] = lines.to_node.apply(
            lambda x: net.bus[net.bus["name"] == x].index[0]
        )
...
def create_pp_ext_grid(net, grid_data):
    if (
        ...
        "hv" in grid_data.target_input.power_levels[0]
        and not grid_data.hv_data.hv_nodes.empty
    ):
        ...
        if not grid_data.components_power.transformers.ehv_hv.empty:
            ...
            nodes_end = find_open_ends(
                grid_data.hv_data.hv_nodes, grid_data.hv_data.hv_lines
            )
